...
namespace MOS::User::App
{
	...
	auto led_test = [](void) {
        while (true) {
            LED_Toggle();
            Task::delay(blink_delay); // Dùng biến toàn cục thay vì số cứng 500_ms
        }
    };

	auto game_listener = [](void) {
        auto& buf = User::Global::stdio.buf; // Lấy buffer UART có sẵn

        while (true) {
            // Chờ nhận dữ liệu từ Python (chờ ký tự xuống dòng '\n')
            // Lưu ý: SyncRxBuf_t trong code của bạn hoạt động theo cơ chế chờ tín hiệu (wait)
            // Ta sẽ giả lập việc đọc dòng đơn giản:
            
            buf.wait(); // Ngủ cho đến khi ISR gọi signal_from_isr() (khi có '\n')
            
            // Đọc dữ liệu ra (đây là giả định hàm pop/read của buffer MOS, 
            // nếu buffer là dạng Queue thì loop pop đến khi hết)
            char c = 0;
            if (!buf.empty()) {
                buf.pop();       
                buf.clear();   
            ...
            // Xử lý lệnh
            if (c == 'F') {
                blink_delay = 100; // Nhanh (Rắn ăn mồi)
            } else if (c == 'S') {
                blink_delay = 500; // Chậm (Reset)
            }
        }
    };
}
...
int main()
{
    ...
    Task::create(App::game_listener, nullptr, 4, "game");
	
	// Shell::add_usr_cmd({"game", [](const char* argv) {
    //     kprintf("<<START_GAME>>\n"); 
    //     LOG("Requesting computer to launch game...");
    // }});

	Shell::add_usr_cmd({"game", [](const char* argv) {
        if (argv[0] == 'b' || argv[0] == 'B') { // game beat
            kprintf("<<START_CYBERBEAT>>\n");
            LOG("Launching game CyberBeat 🎹");
        } 
        else if (argv[0] == 's' || argv[0] == 'S') { // game snake
            kprintf("<<START_CYBERSNAKE>>\n");
            LOG("Launching game CyberSnake 🐍");
        } 
        else {
            LOG("Incorrect Command! Correct: 'game beat' or 'game snake'");
        }
    }});
